...
from src.usr_func.sort_polygon_vertices import sort_polygon_vertices
import numpy as np
...
class GRF:
    ...
    def __discretize_grf_grid(self) -> None:
        ...
        """
        gridsize = self.__GRF_NEIGHBOUR_DISTANCE
        xv = np.arange(0, 1, gridsize)
        yv = np.arange(0, 1, gridsize)
        xx, yy = np.meshgrid(xv, yv)
        self.grid = np.hstack((xx.reshape(-1, 1), yy.reshape(-1, 1)))
